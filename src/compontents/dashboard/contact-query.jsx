...
export default function ContactQueryAdmin() {
  ...
  const fetchItems = useCallback(async () => {
    try {
      ...
      if (!res.ok || !data.success) {
        throw new Error(data.msg || "Failed to fetch contact queries");
      }

      setItems(data.contacts || []);
    } catch (err) {
      toast.error(err.message || "Failed to load contact queries");
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  // ---------- derive helpType filter options from data ----------
  const helpTypeOptions = useMemo(() => {
    const set = new Set(items.map((c) => c.helpType).filter(Boolean));
    return ["All", ...Array.from(set)];
  }, [items]);

  // ---------- client-side search + helpType filter + pagination ----------
  const filteredItems = useMemo(() => {
    ...
    return items.filter((c) => {
      ...
      const matchesHelpType = helpType === "All" || c.helpType === helpType;
      return matchesSearch && matchesHelpType;
    });
  }, [items, search, helpType]);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success(data.msg || "Contact query deleted");
      setDeleteTarget(null);
      await fetchItems();
    } catch (err) {
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mb-6">
          ...
          <p className="mt-1 text-sm text-gray-500">
            {filteredItems.length} {filteredItems.length === 1 ? "query" : "queries"}
          </p>
        </div>
        ...
        <div className="mb-5 flex flex-col gap-3 sm:flex-row">
          ...
          <select
            ...
            className="rounded-xl border border-gray-300 bg-gray-50 px-4 py-2.5 text-sm text-gray-900 outline-none transition focus:border-[#C7954A] focus:ring-2 focus:ring-[#C7954A] sm:w-56"
          >
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50 text-xs uppercase tracking-wide text-gray-500">
                ...
                <th className="px-5 py-3 font-medium">Help Type</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={6}>
                    <EmptyState hasFilters={!!search || helpType !== "All"} />
                  </td>
                </tr>
              ) : (
          ...
          >
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-4">
                <DetailField label="Company Name" value={viewItem.companyName} />
                <DetailField label="Help Type" value={viewItem.helpType} />
                <DetailField label="Phone" value={viewItem.phone} />
          ...
          >
            ...
            >
              ...
              <p className="mt-2 text-sm text-gray-500">
                "{deleteTarget.name}"'s contact query will be permanently
  ...
  return (
    <div className="flex flex-col items-center justify-center px-6 py-16 text-center">
      ...
      <p className="text-sm font-medium text-gray-900">
        {hasFilters ? "No queries match your search" : "No contact queries yet"}
      </p>
      <p className="mt-1 text-sm text-gray-500">
        {hasFilters
          ? "Try a different keyword or help type."
          : "New queries from the website will show up here."}
      </p>
    </div>
  );
}
